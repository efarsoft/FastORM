...
class DatabaseHealthChecker:
    ...
    async def check_engine_health(
        self, engine: AsyncEngine, engine_name: str = "default"
    ) -> HealthCheckResult:
        """检查单个引擎的健康状态
        ...
        try:
            ...
            return HealthCheckResult(
                ...
                response_time=response_time,
                error=str(e),
                details={"engine_name": engine_name, "error_type": type(e).__name__},
            )
    ...
    async def check_all_engines(
        ...
        # 并发检查所有引擎
        tasks = [
            self.check_engine_health(engine, name) for name, engine in engines.items()
        ]
    ...
    def _update_health_status(
        self, engine_name: str, result: HealthCheckResult
    ) -> None:
        """更新引擎健康状态
        ...
        if result.status == HealthStatus.HEALTHY:
            ...
            # 检查是否从故障中恢复
            if (
                engine_name in self._failed_engines
                and self._consecutive_successes[engine_name] >= self.recovery_threshold
            ):
    ...
    def is_engine_healthy(self, engine_name: str) -> bool:
        ...
        Returns:
            是否健康
        """
        return engine_name not in self._failed_engines and self.get_health_status(
            engine_name
    ...
    def get_failover_engine(
        self, preferred_engines: list[str], available_engines: dict[str, AsyncEngine]
    ) -> str | None:
        """获取故障转移引擎
        ...
        for engine_name in preferred_engines:
            if engine_name in available_engines and self.is_engine_healthy(engine_name):
                return engine_name
    ...
    async def start_monitoring(self, engines_getter: callable) -> None:
        """开始监控
        ...
        self._check_task = asyncio.create_task(self._monitoring_loop(engines_getter))
