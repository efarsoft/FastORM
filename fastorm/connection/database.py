...
class Database:
    ...
    _config: ReadWriteConfig | None = None
    _is_read_write_mode: bool = False
    _current_connection_type: ConnectionType | None = None
    ...
    def _init_single_database(cls, database_url: str, **engine_kwargs: Any) -> None:
        ...
        logger.info(f"Database initialized in single mode with {adapter.dialect_name}")

    @classmethod
    def _init_read_write_databases(
        cls, database_urls: dict[str, str], **engine_kwargs: Any
    ) -> None:
        """初始化读写分离数据库连接"""
        required_keys = ["write"]
        ...
        write_engine = create_async_engine(write_final_url, **write_final_config)
        ...
        if "read" in database_urls:
            ...
            read_engine = create_async_engine(read_final_url, **read_final_config)
    ...
    @classmethod
    def _determine_connection_type(cls, operation_hint: str | None = None) -> str:
        """确定连接类型
    ...
    @classmethod
    def get_engine(cls, connection_type: str | None = None) -> AsyncEngine:
        """获取数据库引擎
    ...
    def get_session_factory(
        cls, connection_type: str | None = None
    ) -> async_sessionmaker:
        """获取会话工厂
    ...
    async def session(
        cls, connection_type: str | None = None, force_write: bool = False
    ...
    def get_connection_info(cls) -> dict[str, Any]:
        ...
        return {
            ...
            "read_write_split_enabled": cls._config.enable_read_write_split
            if cls._config
            else False,
            "current_connection_type": cls._current_connection_type.value
            if cls._current_connection_type
            else None,
        }
