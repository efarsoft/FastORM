...
class ChineseProvider(BaseProvider):
    ...
    chinese_given_names = [
        ...
        '刚', '桂英', '华', '建华', '建国', '志强', '志明', '秀兰',
        '秀珍', '春梅', '玉兰', '玉梅', '桂芳', '秀芳', '丽娟', '丽华'
    ]
...
class CompanyProvider(BaseProvider):
    ...
    # 部门名称
    departments = [
        '技术部', '产品部', '市场部', '销售部', '运营部', '人力资源部',
        '财务部', '法务部', '行政部', '客服部', '设计部', '测试部',
        '运维部', '数据部', '业务部', '项目部', '质量部', '采购部'
    ]
    
    # 职位名称
    positions = [
        '软件工程师', '高级软件工程师', '架构师', '技术经理', '技术总监',
        '产品经理', '高级产品经理', '产品总监', '项目经理', '运营经理',
        '市场经理', '销售经理', '人力资源经理', '财务经理', 'UI设计师',
        '测试工程师', '运维工程师', '数据分析师', '业务分析师'
    ]
    
    # 技能标签
    skills = [
        'Python', 'Java', 'JavaScript', 'React', 'Vue', 'Node.js',
        'Docker', 'Kubernetes', 'AWS', 'Azure', 'MySQL', 'PostgreSQL',
        'Redis', 'MongoDB', 'Git', 'Jenkins', 'Linux', 'Nginx'
    ]
    ...
    def company_email(self, name: Optional[str] = None) -> str:
        """生成企业邮箱"""
        if not name:
            name = self.generator.first_name().lower()
        
        domains = [
            'company.com', 'corp.com', 'tech.com', 'group.com',
            'ltd.com', 'inc.com', 'enterprise.com'
        ]
...
def company_email(name: Optional[str] = None) -> str:
    """生成企业邮箱"""
