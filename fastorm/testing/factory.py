...
class FactoryMetaclass(type):
    ...
    def __new__(mcs, name: str, bases: tuple, namespace: dict[str, Any], **kwargs):
        ...
        for key, value in list(namespace.items()):
            ...
            # 处理特征方法
            if callable(value) and hasattr(value, "_is_trait") and value._is_trait:
...
class Factory(metaclass=FactoryMetaclass):
    ...
    @classmethod
    async def create(cls, trait: str | None = None, **overrides: Any) -> T:
        """创建单个模型实例
    ...
    async def create_batch(
        cls, count: int, trait: str | None = None, **overrides: Any
    ) -> list[T]:
        """批量创建模型实例
    ...
    @classmethod
    async def build(cls, trait: str | None = None, **overrides: Any) -> T:
        """构建模型实例但不保存到数据库
    ...
    async def _build_attributes(
        cls, trait: str | None = None, **overrides: Any
    ) -> dict[str, Any]:
        """构建属性字典
...
class SubFactory:
    ...
    def __init__(
        self, factory_class: type[Factory], trait: str | None = None, **kwargs: Any
    ):
    ...
    async def create(self) -> Any:
        """创建子对象"""
        return await self.factory_class.create(trait=self.trait, **self.kwargs)
