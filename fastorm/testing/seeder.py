...
class Seeder(ABC, metaclass=SeederMetaclass):
    ...
    async def call(
        self, seeder_class: type[Seeder], verbose: bool = True, **kwargs: Any
    ) -> None:
        """调用其他Seeder
...
class DatabaseSeeder(Seeder):
    ...
    def __init__(self, seeders: list[type[Seeder]] | None = None):
        super().__init__()
    ...
    async def run_parallel(self, max_concurrent: int = 3, verbose: bool = True) -> None:
        """并行执行Seeder
...
# 便捷函数
async def run_seeder(seeder_class: type[Seeder], verbose: bool = True) -> None:
    """运行单个Seeder
...
async def run_all_seeders(
    parallel: bool = False, max_concurrent: int = 3, verbose: bool = True
) -> None:
    """运行所有注册的Seeder
