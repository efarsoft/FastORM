...
@click.option('--output', '-o', default='app/models',
              help='输出目录 (默认: app/models)')
@click.option('--force', is_flag=True, help='覆盖已存在的文件')
@click.pass_context
def create_model(ctx, model_name: str, table: str, fields: tuple, 
    ...
    if not _is_valid_model_name(model_name):
        click.echo("❌ 模型名称无效。请使用PascalCase格式，如: User, BlogPost", err=True)
        sys.exit(1)
    ...
    try:
        ...
        # 提示下一步操作
        click.echo(f"\n📚 下一步操作:")
        click.echo(f"   fastorm migrate          # 生成并运行数据库迁移")
...
def _generate_model_code(model_name: str, table_name: str, 
