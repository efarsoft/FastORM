...
@click.option('--output-dir', '-o', default='fastorm_models',
              help='输出目录')
...
@click.pass_context
def convert(ctx, source_path: str, output_dir: str, backup: bool, force: bool, dry_run: bool):
    """
    ...
    try:
        ...
        for model_file in model_files:
            result = _convert_model_file(model_file, output_path, backup, force, dry_run, verbose)
...
def _is_sqlalchemy_model_file(file_path: Path) -> bool:
    ...
    try:
        ...
        return any(re.search(pattern, content, re.IGNORECASE) for pattern in sqlalchemy_patterns)
        
    except Exception:
        return False


def _convert_model_file(model_file: Path, output_path: Path, backup: bool, 
...
def _analyze_model_class(class_node: ast.ClassDef, source_content: str) -> Optional[Dict]:
    """分析模型类"""
    model_info = {
        ...
        if isinstance(target, ast.Name):
            ...
            if target_name == '__tablename__':
                ...
                if _is_column_call(assign_node.value):
                    # Column定义
                    column_info = _analyze_column(target_name, assign_node.value, source_content)
                    model_info['columns'].append(column_info)
                elif _is_relationship_call(assign_node.value):
                    # Relationship定义
                    rel_info = _analyze_relationship(target_name, assign_node.value, source_content)
...
def _analyze_relationship(rel_name: str, call_node: ast.Call, source_content: str) -> Dict:
    """分析relationship定义"""
    rel_info = {
        'name': rel_name,
        'target': None,
...
def _generate_fastorm_code(models: List[Dict], original_content: str, verbose: bool) -> str:
    """生成FastORM代码"""
    lines = []
