...
from typing import Any, Dict, ClassVar, Type, Optional, List, TYPE_CHECKING
...
from .loader import RelationProxy, RelationLoader
...
class RelationProxy:
    ...
    # 多对多关系的方法
    async def attach(self, ids: Any, pivot_data: Optional[Dict[str, Any]] = None) -> None:
        """附加关联记录（多对多）"""
    ...
    async def detach(self, ids: Optional[Any] = None) -> None:
        """分离关联记录（多对多）"""
    ...
    async def sync(self, ids: List[Any], pivot_data: Optional[Dict[str, Any]] = None) -> None:
        """同步关联记录（多对多）"""
    ...
    async def toggle(self, ids: Any, pivot_data: Optional[Dict[str, Any]] = None) -> Dict[str, List[int]]:
        """切换关联状态（多对多）"""
...
class RelationMixin:
    ...
    def belongs_to_many(
        ...
        return BelongsToMany(
            model_class, pivot_table, foreign_key, related_key,
            local_key, related_local_key
        )
    ...
    async def load_relations(self, *relation_names: str) -> Dict[str, Any]:
        ...
        # 构建关系字典
        relations = {
            name: relation for name, relation in self.__relations__.items()
