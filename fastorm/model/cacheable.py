...
class CacheableModel:
    ...
    _cache_tags: Optional[Set[str]] = None
    ...
    async def cache_instance(self, ttl: Optional[int] = None) -> None:
        """缓存当前模型实例
    ...
    @classmethod
    async def find_cached(cls: type[T], id: Any) -> Optional[T]:
        """从缓存中查找模型实例
    ...
    @classmethod
    async def find_or_cache(cls: type[T], id: Any, ttl: Optional[int] = None) -> Optional[T]:
    ...
    def _get_cache_tags(self) -> Optional[Set[str]]:
        """获取模型的缓存标签
    ...
    @classmethod
    def _deserialize_instance(cls: type[T], cached_data: Dict[str, Any]) -> Optional[T]:
        """反序列化缓存的模型实例
        ...
        if model_class != cls.__name__:
            logger.warning(f"缓存数据类型不匹配: 期望 {cls.__name__}, 实际 {model_class}")
            return None
