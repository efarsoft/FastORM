        ...
        async with self._semaphore:
            try:
                ...
                return await asyncio.wait_for(
                    self._serialize_async_impl(obj, config, context),
                    timeout=timeout
                )
    ...
    def _serialize_pydantic_model(
        ...
        try:
            ...
            return model.model_dump(
                include=None,
                exclude=context.config.exclude_relations if not context.config.serialize_relations else None,
            ...
            if hasattr(e, 'errors'):
                raise convert_pydantic_serialization_error(e, {
                    'model': model.__class__.__name__,
                    'context': context.model_name
                })
            else:
                raise SerializationError(
                    f"Pydantic模型序列化失败: {str(e)}",
        ...
        for column in model.__table__.columns:
            ...
            if field_name in self._field_serializers:
                try:
                    ...
                    context.add_error(SerializationError(
                        f"字段序列化失败: {str(e)}",
                        field=field_name,
