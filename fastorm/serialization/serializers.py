...
class ModelSerializer(BaseSerializer):
    ...
    def __init__(
        ...
        name: str,
        model_type: Type,
        field_serializers: Optional[Dict[str, FieldSerializer]] = None,
    ...
    def _get_model_fields(self, model: Any) -> List[str]:
        ...
        # 其他类型，使用__dict__
        return [key for key in model.__dict__.keys() 
    ...
    def _serialize_field_value(
        ...
        if isinstance(value, (list, tuple, set)):
            return [self._serialize_field_value(item, field_name, context) 
                    for item in value]
        
        if isinstance(value, dict):
            return {k: self._serialize_field_value(v, field_name, context) 
...
class RelationSerializer(BaseSerializer):
    ...
    def __init__(
        ...
        target_serializer: Optional[BaseSerializer] = None,
        lazy: bool = True,
        max_depth: int = 2,
        description: str = ""
    ):
    ...
    def _serialize_single_relation(self, value: Any, context: Any = None) -> Dict[str, Any]:
        """序列化单个关系对象"""
    ...
    def _default_serialize_object(self, obj: Any, context: Any = None) -> Dict[str, Any]:
        """默认对象序列化"""
...
class SerializerRegistry:
    ...
    def get_field_serializer(self, field_name: str) -> Optional[FieldSerializer]:
        """获取字段序列化器"""
        return self._field_serializers.get(field_name)
    
    def get_model_serializer(self, model_type: Type) -> Optional[ModelSerializer]:
        """获取模型序列化器"""
        return self._model_serializers.get(model_type)
    
    def get_relation_serializer(self, relation_name: str) -> Optional[RelationSerializer]:
        """获取关系序列化器"""
        return self._relation_serializers.get(relation_name)
    
    def get_type_serializer(self, value_type: Type) -> Optional[BaseSerializer]:
        """获取类型序列化器"""
