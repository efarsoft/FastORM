...
from .exceptions import ModelValidationError, FieldValidationError
...
class ModelValidatorRegistry:
    ...
    def register(
        ...
        logger.debug(f"Registered model validator '{validator_name}' for model '{model_name}'")
    
    def get_validators(self, model_name: str) -> List[tuple]:
        """获取模型的所有验证器"""
        return self._validators.get(model_name, [])
    
    def get_config(self, model_name: str, validator_name: str) -> Optional[ModelValidatorConfig]:
        """获取验证器配置"""
        config_key = f"{model_name}:{validator_name}"
    ...
    def validator(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if comparison == "equal":
                ...
                valid = value1 >= value2
                error_message = f"Field '{field1}' must be greater than or equal to '{field2}'"
            elif comparison == "less_equal":
                valid = value1 <= value2
                error_message = f"Field '{field1}' must be less than or equal to '{field2}'"
            else:
            ...
            raise ModelValidationError(
                model_name=cls.__name__ if hasattr(cls, '__name__') else "Unknown",
                message=f"Cannot compare fields '{field1}' and '{field2}': {str(e)}",
                context={
                    "field1": field1,
                    "field2": field2,
                    "value1": value1,
...
class ModelValidatorChain:
    ...
    def add_field_comparison(
        ...
        """添加字段比较验证"""
        return self.add_validator(
            validate_field_comparison(field1, field2, comparison)
        )
