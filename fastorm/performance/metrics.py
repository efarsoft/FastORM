...
class SystemMetrics:
    ...
    cpu_percent: float
    cpu_count: int
    load_average: list[float] | None  # 1, 5, 15分钟平均负载
...
class MetricsCollector:
    ...
    def get_memory_metrics(self, last_minutes: int = 60) -> list[MemoryMetrics]:
        ...
        with self._lock:
            return [m for m in self._memory_metrics if m.timestamp > cutoff]

    def get_system_metrics(self, last_minutes: int = 60) -> list[SystemMetrics]:
        """获取系统指标"""
        cutoff = datetime.now() - timedelta(minutes=last_minutes)
        with self._lock:
            return [m for m in self._system_metrics if m.timestamp > cutoff]

    def get_connection_metrics(
        self, last_minutes: int = 60
    ) -> list[ConnectionPoolMetrics]:
        """获取连接池指标"""
        cutoff = datetime.now() - timedelta(minutes=last_minutes)
        with self._lock:
            return [m for m in self._connection_metrics if m.timestamp > cutoff]

    def get_cache_metrics(self, last_minutes: int = 60) -> list[CacheMetrics]:
        """获取缓存指标"""
        cutoff = datetime.now() - timedelta(minutes=last_minutes)
        with self._lock:
            return [m for m in self._cache_metrics if m.timestamp > cutoff]
...
def record_cache_performance(
    total_requests: int, cache_hits: int, cache_size: int, memory_usage: float = 0.0
) -> None:
    """记录缓存性能指标"""
