...
from .detector import N1QueryAlert
from .detector import QueryPattern
from .profiler import QueryInfo
...
class PerformanceMonitor:
    ...
    def analyze_query(
        self, sql: str, execution_time: float, error: str | None = None
    ) -> None:
        """分析单个查询"""
    ...
    def get_slow_queries(self, limit: int = 10) -> list[QueryInfo]:
        ...
        # 筛选慢查询并排序
        slow_queries = [q for q in all_queries if q.is_slow and q.duration is not None]
    ...
    def get_failed_queries(self, limit: int = 10) -> list[QueryInfo]:
        ...
        # 筛选失败查询并排序
        failed_queries = [q for q in all_queries if not q.is_success]
        failed_queries.sort(key=lambda x: x.start_time, reverse=True)

        return failed_queries[:limit]

    def get_n1_alerts(self, severity: str | None = None) -> list[N1QueryAlert]:
        """获取N+1查询警告"""
...
def analyze_query(sql: str, execution_time: float, error: str | None = None) -> None:
    """分析查询性能"""
