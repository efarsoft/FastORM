...
class N1Detector:
    ...
    def analyze_query(self, sql: str, execution_time: Optional[datetime] = None) -> None:
        """分析查询是否为N+1模式"""
    ...
    def _trigger_alert(self, pattern: QueryPattern, triggered_at: datetime) -> None:
        ...
        recent_alerts = [
            alert for alert in self.alerts[-10:]  # 检查最近10个警告
            if (alert.pattern.sql_template == pattern.sql_template and
                triggered_at - alert.triggered_at < timedelta(minutes=5))
        ]
    ...
    def _generate_suggestions(self, pattern: QueryPattern) -> List[str]:
        ...
        if "SELECT" in sql and "WHERE" in sql:
            suggestions.append(
                "考虑使用 eager loading (with_关系名) 预加载关联数据"
            )
            suggestions.append(
                "检查是否可以用 JOIN 查询替代多次单独查询"
            )
        
        if "IN (" in sql:
            suggestions.append(
                "考虑批量查询：将多个 IN 查询合并为一个"
            )
    ...
    def get_alerts(self, severity: Optional[str] = None) -> List[N1QueryAlert]:
        """获取警告列表"""
...
def detect_n1_queries(sql: str, execution_time: Optional[datetime] = None) -> None:
    """便捷的N+1查询检测函数"""
    _global_detector.analyze_query(sql, execution_time)


def get_n1_alerts(severity: Optional[str] = None) -> List[N1QueryAlert]:
    """获取N+1查询警告"""
