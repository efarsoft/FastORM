...
class BatchOperation(ABC):
    ...
    def __init__(self, model_class: type, config: dict[str, Any] | None = None):
...
class BatchUpdate(BatchOperation):
    ...
    async def execute(self, session, data: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for record in data:
            ...
            update_data = {
                k: v for k, v in record.items() if k not in self.where_fields
            }
...
class BatchDelete(BatchOperation):
    ...
    async def execute(
        self, session, conditions: list[dict[str, Any]]
    ) -> dict[str, Any]:
        """执行批量删除"""
        deleted_count = 0
...
class BatchUpsert(BatchOperation):
    ...
    async def execute(self, session, data: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        stmt = stmt.on_conflict_do_update(
            index_elements=self.conflict_fields, set_=update_dict
        )
