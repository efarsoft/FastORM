...
class BatchValidationError(BatchError):
    ...
    def __init__(self, message: str, validation_errors: list[dict[str, Any]], **kwargs):
...
class BatchTimeoutError(BatchError):
    ...
    def __str__(self) -> str:
        base_msg = super().__str__()
        return (
            f"{base_msg} (超时: {self.timeout_seconds}s, "
...
class BatchMemoryError(BatchError):
    ...
    def __str__(self) -> str:
        ...
        if self.memory_used_mb and self.memory_limit_mb:
            base_msg += (
                f" (内存使用: {self.memory_used_mb:.1f}MB/"
...
class BatchIntegrityError(BatchError):
    ...
    def __init__(
        ...
        violating_records: list[dict[str, Any]] | None = None,
        **kwargs,
    ):
        super().__init__(message, **kwargs)
...
class BatchConcurrencyError(BatchError):
    ...
    def __str__(self) -> str:
        ...
        if self.concurrent_operations and self.max_concurrent:
            base_msg += (
                f" (并发操作: {self.concurrent_operations}/" f"{self.max_concurrent})"
            )
        return base_msg
...
def convert_batch_error(
    ...
    if isinstance(error, SQLAlchemyError):
        ...
        if "integrity constraint" in error_message.lower():
            return BatchIntegrityError(
                f"数据完整性错误: {error_message}", operation=operation, context=context
            )
        ...
        # 其他SQLAlchemy错误
        return BatchError(
            f"数据库操作错误: {error_message}", operation=operation, context=context
        )

    # 内存错误
    if isinstance(error, MemoryError):
        return BatchMemoryError(
            f"内存不足: {error!s}", operation=operation, context=context
        )
    ...
    if isinstance(error, PermissionError):
        return BatchPermissionError(
            f"权限不足: {error!s}", operation=operation, context=context
        )

    # 通用错误
    return BatchError(f"批量操作失败: {error!s}", operation=operation, context=context)


def format_batch_errors(
    errors: list[BatchError], include_details: bool = True
) -> dict[str, Any]:
    """格式化批量错误列表"""
...
def get_error_recovery_suggestions(error: BatchError) -> list[str]:
    ...
    if isinstance(error, BatchValidationError):
        suggestions.extend(
            ["检查输入数据格式和完整性", "验证必填字段是否完整", "确认数据类型匹配"]
        )

    elif isinstance(error, BatchTransactionError):
        suggestions.extend(
            ["检查数据库连接状态", "减少批量大小以降低事务复杂度", "考虑使用分块处理"]
        )
        ...
        suggestions.extend(
            ["检查数据完整性约束", "验证外键关系", "确认唯一性约束", "预处理重复数据"]
        )

    elif isinstance(error, BatchConcurrencyError):
        suggestions.extend(["减少并发操作数量", "增加重试机制", "使用排队系统处理请求"])

    else:
        suggestions.extend(
            ["检查错误详情和上下文", "验证输入参数", "查看系统日志获取更多信息"]
        )
