...
import time
import psutil
from typing import Any, Dict, List, Optional, Type, Union, AsyncGenerator, Iterator
...
class BatchContext:
    ...
    current_memory_mb: float = field(default_factory=lambda: psutil.Process().memory_info().rss / 1024 / 1024)
...
class BatchEngine:
    ...
    def __init__(self, session: Union[Session, AsyncSession], config: Optional[BatchConfig] = None):
    ...
    async def batch_delete(
        self,
        model_class: Type,
        conditions: Union[List[Dict[str, Any]], AsyncGenerator[Dict[str, Any], None]],
        config: Optional[BatchConfig] = None
    ) -> Dict[str, Any]:
        """批量删除操作"""
    ...
    async def _async_batch_insert(
        ...
        try:
            async with self._async_transaction_context(context):
                ...
                # 分批处理
                async for batch in self._chunk_async_data(data_iterator, context.config.batch_size):
            ...
            error = convert_batch_error(e, "async_batch_insert", {
    ...
    def _sync_batch_insert(
        ...
        try:
            with self._sync_transaction_context(context):
                ...
                # 分批处理
                for batch in self._chunk_sync_data(data_iterator, context.config.batch_size):
            ...
            error = convert_batch_error(e, "sync_batch_insert", {
                'model': context.model_class.__name__,
                'table': context.table_name
            })
            context.add_error(error)
        ...
        try:
            if context.config.bulk_insert_mappings:
                ...
                await self.session.execute(
                    insert(context.model_class.__table__),
                    batch
                )
            else:
                # 使用传统方式
    ...
    async def _chunk_async_data(
        self,
        data_iterator: AsyncGenerator[Dict[str, Any], None],
        chunk_size: int
    ...
    def _build_result(self, context: BatchContext) -> Dict[str, Any]:
        ...
        processing_rate = context.processed_records / elapsed_time if elapsed_time > 0 else 0
        ...
        self._stats['average_processing_rate'] = (
            ...
            if self._stats['total_time_spent'] > 0 else 0
        )
        ...
        return {
            ...
            'success_rate': (
                (context.processed_records - context.failed_records) / context.processed_records * 100
                if context.processed_records > 0 else 0
            ),
            'elapsed_time': elapsed_time,
            ...
            } if context.transaction_id else None,
