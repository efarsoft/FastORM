...
class QueryCacheSupport:
    ...
    def __init__(self):
        ...
        self._cache_tags: Optional[Set[str]] = None
        self._cache_driver: Optional[str] = None
    ...
    def forget(self, key: Optional[str] = None) -> 'QueryBuilder':
        """清除查询缓存
    ...
    def _generate_cache_key(self) -> str:
        ...
        if hasattr(self, '_wheres') and self._wheres:
            ...
            query_parts.append(f"where:{hashlib.md5(where_str.encode()).hexdigest()[:8]}")
        
        # 添加ORDER BY
        if hasattr(self, '_orders') and self._orders:
            order_str = json.dumps(self._orders, sort_keys=True, default=str)
            query_parts.append(f"order:{hashlib.md5(order_str.encode()).hexdigest()[:8]}")
        ...
        if hasattr(self, '_columns') and self._columns:
            ...
            query_parts.append(f"select:{hashlib.md5(columns_str.encode()).hexdigest()[:8]}")
...
class CacheableQueryBuilder:
    ...
    # 代理缓存相关方法
    def remember(self, ttl: int = 300, key: Optional[str] = None, 
                tags: Optional[Union[str, List[str], Set[str]]] = None):
