...
if TYPE_CHECKING:
    ...
    from fastorm.query.pagination import SimplePaginator
...
class QueryBuilder(Generic[T]):
    ...
    async def chunk(
        self, size: int, callback: Callable[[list[T]], Any], preserve_order: bool = True
    ) -> int:
        """分块处理数据的便捷方法
    ...
    async def each(self, callback: Callable[[T], Any], chunk_size: int = 100) -> int:
        """逐个处理记录的便捷方法
    ...
    async def _load_relations(self, instances: list[T], session: AsyncSession) -> None:
        """加载预定义的关系
    ...
    def with_(self, relations: str | list[str]) -> QueryBuilder[T]:
        """预加载关系（防止N+1查询）
    ...
    def where_has(
        self,
        relation: str,
        callback: Callable[[QueryBuilder], QueryBuilder] | None = None,
        operator: str = ">=",
        count: int = 1,
    ) -> QueryBuilder[T]:
        ...
        if hasattr(self._model_class, relation):
            ...
            if hasattr(relation_obj, "model_class"):
                ...
                if hasattr(relation_obj, "get_foreign_key"):
                    ...
                    # 构建EXISTS子查询
                    subquery = subquery_builder._query.where(
                        getattr(related_model, foreign_key)
