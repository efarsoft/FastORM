...
def invalidate_cache(
    tags: Optional[Union[str, Set[str]]] = None,
    keys: Optional[Union[str, Set[str]]] = None,
...
def _normalize_tags(tags: Optional[Union[str, Set[str]]]) -> Optional[Set[str]]:
    """标准化标签"""
    if not tags:
        return None
...
def _deserialize_if_needed(cached_result: Any, serialize: bool) -> Any:
    ...
    if (isinstance(cached_result, str) and 
        cached_result.startswith('{')):
        try:
...
async def _invalidate_caches(
    tags: Optional[Union[str, Set[str]]],
    keys: Optional[Union[str, Set[str]]],
    clear_all: bool
) -> None:
    """失效缓存的通用方法"""
    try:
