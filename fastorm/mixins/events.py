...
class EventHandler:
    ...
    def __init__(
        self, handler: Callable, priority: int = 0, condition: Callable | None = None
    ):
...
class EventMixin:
    ...
    def add_event_handler(
        ...
        # 按优先级排序
        cls._event_handlers[event].sort(key=lambda h: h.priority, reverse=True)
    ...
    def remove_event_handler(cls, event: str, handler: Callable):
        ...
        if event in cls._event_handlers:
            cls._event_handlers[event] = [
                h for h in cls._event_handlers[event] if h.handler != handler
            ]
        ...
        for event_handler in self._event_handlers[event]:
            ...
            try:
                ...
                if event in critical_events:
                    # 验证类事件失败应该中断操作
                    is_validator = (
                        "validate" in handler_name.lower()
                        or "check" in handler_name.lower()
                    )
                    if is_validator:
    ...
    def _save_original_state(self):
        ...
        if hasattr(self, "__table__") and not self._original_state_saved:
            ...
            for column in self.__table__.columns:
                self._model_state[column.name] = getattr(self, column.name, None)
    ...
    @classmethod
    def on_event(cls, event: str, priority: int = 0, condition: Callable | None = None):
        """事件处理器装饰器
...
def before_insert(priority: int = 0, condition: Callable | None = None):
    """新增前事件装饰器"""
...
def after_insert(priority: int = 0, condition: Callable | None = None):
    """新增后事件装饰器"""
...
def before_update(priority: int = 0, condition: Callable | None = None):
    """更新前事件装饰器"""
...
def after_update(priority: int = 0, condition: Callable | None = None):
    """更新后事件装饰器"""
...
def before_delete(priority: int = 0, condition: Callable | None = None):
    """删除前事件装饰器"""
...
def after_delete(priority: int = 0, condition: Callable | None = None):
    """删除后事件装饰器"""
